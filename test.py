...
# mage_dir = r"C:\Users\MD Sahil Johan Islam\OneDrive\Desktop\P1\R3\Backend\plantvillage dataset\color\Apple___Apple_scab"
...
# Test: All images in dataset (recursive)
def test_all_images():
    i = 1
    for root, _, files in os.walk(image_dir):  # walks through subfolders too
        for image_name in files:
            ...
            try:
                with open(image_path, "rb") as f:
                    ...
                    response = requests.post(url, files=files)
                    response.raise_for_status()
                    print(f"[Image {i}] {image_name} → {response.json()}")
            except requests.exceptions.RequestException as e:
                print(f"[Image {i}] {image_name} → Request failed: {e}")
            except Exception as e:
